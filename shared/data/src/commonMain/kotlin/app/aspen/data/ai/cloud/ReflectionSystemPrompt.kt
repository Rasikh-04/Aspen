...
/**
 * The Tier-2 system prompt (docs/04 ADR-003, docs/06 §5) — an ADVISOR-REVIEW SURFACE, not ordinary
 * code (docs/07 Phase 4 [APPROVE]; tracked in docs/PRE_SHIP_VERIFICATION.md). It is the FRONT LINE
 * of SR-3; [app.aspen.domain.safety.SafetyEngine.guardOutput] and the crisis-sign input check remain
 * the enforced backstops regardless of what the model does with these instructions.
 *
 * Not user-facing copy (it is sent to the API, never rendered), so it lives in code, in English,
 * and is exempt from the string-resource rule — but NOT from review.
 */
